...
function redraw(name){
  ...
  var selectedGlacier = ee.Feature(glacs.filter(ee.Filter.eq('group_code', name)).first());
  Map.centerObject(selectedGlacier, 10);
  ...
  function showChart(year) {
    ...
    var selectedGlacChart = ui.Chart.image.series(albedo, selectedGlacier)
...
// Get the glacier names
var glacNames = ee.List(glacs.aggregate_array('group_code'));

// Initialize glacier selector
var selectGlac = ui.Select({
  items: glacNames.getInfo(), 
  onChange: redraw,
  style: {position: 'top-center', padding: '7px'}}
  );

selectGlac.setPlaceholder('Choose a Glacier ...'); 
