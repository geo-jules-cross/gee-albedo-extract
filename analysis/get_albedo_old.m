...
albedo.glaciers.names = {'TAR', 'SUS', 'CAA', 'COH', 'CRS', 'HOD',... %leave first six in order for plots
                         'RHO', 'LCX', 'MTN', 'BNS', 'SLS', 'WLS'};
...
for g=1:12
    ...
    glacier = vertcat(...
        table2cell(readtable([exportDirectory ['alb_old_extract_' name '_05.csv']],...
        'Delimiter', ',', 'HeaderLines', 1, 'ReadVariableNames', false, 'Format', '%D %f %f %f %s %s %s %f %s %s')),...
        table2cell(readtable([exportDirectory ['alb_old_extract_' name '_10.csv']],...
        'Delimiter', ',', 'HeaderLines', 1, 'ReadVariableNames', false, 'Format', '%D %f %f %f %s %s %s %f %s %s')),...
        table2cell(readtable([exportDirectory ['alb_old_extract_' name '_15.csv']],...
        'Delimiter', ',', 'HeaderLines', 1, 'ReadVariableNames', false, 'Format', '%D %f %f %f %s %s %s %f %s %s')));
    ...
    for d=1:length(albedo.TAR.date)
        ...
        if (length(dayind)==1) % we have an entry for this day
            if(glacier{dayind,2} > 0)
                ...
                albedo.glaciers.swirWSA(d,g)=glacier{dayind,4}/1000;
                albedo.glaciers.swir(d,g)=(((glacier{dayind,2}+glacier{dayind,4}))/2)/1000;
...
save([dataDir 'albedo_old.mat'], 'albedo');