...
%% Single Year TARM v. MODIS Plots - New
...
for year=1:length(years)
    ...
    set(gcf, 'Units', 'inches', 'OuterPosition', [2, 0.6, 10, 4]);
    ...
    plot(albedo.new.TAR.date(strt:fnsh), albedo.new.TAR.swirsmooth(strt:fnsh), 'o', 'LineWidth',2, 'MarkerSize',2);
    plot(albedo.new.TAR.date(strt:fnsh), albedo.new.TAR.swir(strt:fnsh), ':','Color',[0.5 0.5 0.5], 'LineWidth',1.5);
    ...
    % Validate w/ RMSE
    x = albedo.new.TAR.measured(strt:fnsh);
    y = albedo.new.TAR.swir(strt:fnsh);
    err = (x - y);
...
%% Single Year CAAM v. MODIS Plots - New
...
for year=1:length(years)
    ...
    strt=find(albedo.new.CAA.date==datenum([yr 11 1]));
    fnsh=find(albedo.new.CAA.date==datenum([yr+1 2 28]));
    
    figure(200+year); clf;
    set(gcf, 'Name', ['fig-CAAM-stn-', num2str(yr)]);
    
    set(gcf, 'Units', 'inches', 'OuterPosition', [2, 0.8, 10, 4]);
    ...
    plot(albedo.new.CAA.date(strt:fnsh), albedo.new.CAA.swirsmooth(strt:fnsh), 'o', 'LineWidth',2, 'MarkerSize',2);
    plot(albedo.new.CAA.date(strt:fnsh), albedo.new.CAA.swir(strt:fnsh), ':','Color',[0.5 0.5 0.5], 'LineWidth',1.5);
    ...
    % Validate w/ RMSE
    x = albedo.new.CAA.measured(strt:fnsh);
    y = albedo.new.CAA.swir(strt:fnsh);
    err = (x - y);
...
%% Single Year Snow HODM v. MODIS Plots - New
...
for year=1:length(years)
    ...
    strt=find(albedo.new.HOD.date==datenum([yr 11 1]));
    fnsh=find(albedo.new.HOD.date==datenum([yr+1 2 28]));
    
    figure(300+year); clf;
    set(gcf, 'Name', ['fig-HODM-stn-', num2str(yr)]);
    
    set(gcf, 'Units', 'inches', 'OuterPosition', [2, 0.5, 10, 4]);
    ...
    plot(albedo.new.HOD.date(strt:fnsh), albedo.new.HOD.swirsmooth(strt:fnsh), 'o', 'LineWidth',2, 'MarkerSize',2);
    plot(albedo.new.HOD.date(strt:fnsh), albedo.new.HOD.swir(strt:fnsh), ':','Color',[0.5 0.5 0.5], 'LineWidth',1.5);
    
    shadedErrorBar(albedo.new.HOD.date(strt:fnsh),...
    ...
    % Validate w/ RMSE
    x = albedo.new.HOD.measured(strt:fnsh);
    y = albedo.new.HOD.swir(strt:fnsh);
    err = (x - y);
...
%% Single Year COHM v. MODIS Plots - New
...
for year=1:length(years)
    ...
    strt=find(albedo.new.COH.date==datenum([yr 11 1]));
    fnsh=find(albedo.new.COH.date==datenum([yr+1 2 28]));
    ...
    plot(albedo.new.COH.date(strt:fnsh), albedo.new.COH.swirsmooth(strt:fnsh), 'o', 'LineWidth',2, 'MarkerSize',2);
    plot(albedo.new.COH.date(strt:fnsh), albedo.new.COH.swir(strt:fnsh), ':','Color',[0.5 0.5 0.5], 'LineWidth',1.5);
    
    shadedErrorBar(albedo.new.COH.date(strt:fnsh),...
    ...
    % Validate w/ RMSE
    x = albedo.new.COH.measured(strt:fnsh);
    y = albedo.new.COH.swir(strt:fnsh);
    err = (x - y);
...
%% Single Year TARM v. Glacier Plots - Old
...
%% Single Year CAAM v. Glacier Plots - Old
...
%% Single Year HODM v. Glacier Plots - Old
...
%% Single Year COHM v. Glacier Plots - Old
